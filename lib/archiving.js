...
*   The archive has an audio track
*
* @param {Boolean} hasVideo
*   The archive has a video track
*
* @param {String} url
...
var api = function(config, method, path, body, scope, claims, callback) {
  ...
  request({
    ...
    method: method,
    headers: {
      'X-OPENTOK-AUTH': auth.getAuthToken(scope, config, claims),
...
exports.listArchives = function(config, options, callback) {
  ...
  api(config, 'GET', '/archive?' + qs.join('&'), null, 'archive.list', null, function(err, response, body) {
    if(!err && body ) {
      try {
...
exports.startArchive = function(config, sessionId, options, callback) {
  ...
  api(config, 'POST', '/archive', {
    sessionId: sessionId,
    name: options.name
  }, 'archive.create', {sid: sessionId}, function(err, response, body) {
    if(err) {
      callback(err);
...
exports.stopArchive = function(config, archiveId, callback) {
  ...
  api(config, 'POST', '/archive/' + encodeURIComponent(archiveId) + '/stop', {},
    'archive.stop', null, function(err, response, body) {
    if(err) {
      callback(err);
    } else if(response.statusCode != 200) {
      if(response && response.statusCode == 404) {
...
exports.getArchive = function(config, archiveId, callback) {
  ...
  api(config, 'GET', '/archive/' + archiveId, null, 'archive', null,
...
exports.deleteArchive = function(config, archiveId, callback) {
  ...
  api(config, 'DELETE', '/archive/' + encodeURIComponent(archiveId), null,
    'archive.delete', null, function(err, response) {
